...
SELECT * FROM CHANNEL;
--채널 수정(updateChannel)
UPDATE CHANNEL SET CHANNEL_NAME = '장밥두비두밥' WHERE CHANNEL_CODE =1;
COMMIT;
SELECT * FROM CHANNEL;
--채널 삭제(deleteChannel)
DELETE FROM CHANNEL WHERE CHANNEL_CODE = 3;
COMMIT;
SELECT * FROM CHANNEL;
--내 채널 보기(myChannel)
...
VALUES(SEQ_VIDE.NEXTVAL, '봄 여름 가을 겨울', 'URL01','PHOTO01',1,1,'user1');
COMMIT;
--비디오 수정
UPDATE VIDEO SET VIDEO_TITLE = '살려줘....' WHERE VIDEO_CODE = 1;
-- 비디오 삭제
...
SELECT * FROM VIDEO;
-- 채널별 목록보기
SELECT * FROM VIDEO WHERE CHANNEL_CODE=4;
-- 비디오 1개 보기
