...
/*
    ...
    * VIEW 옵션들
    - OR REPLACE : 뷰 생성시 기존에 중복된 이름의 뷰가 없다면
                   새로이 뷰를 생성하고, 기존에 중복된 이름의 뷰가 있다면 해당 뷰를 변경(갱신)하는 옵션
...
*/
-- 관리자 계정으로 CREATE VIEW 권한을 부여할 수 있음
...
SELECT EMP_ID, EMP_NAME,JOB_NAME, DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남',2, '여') AS 성별, EXTRACT(YEAR FROM SYSDATE) -EXTRACT(YEAR FROM HIRE_DATE)||'년'
FROM EMPLOYEE
...
AS SELECT EMP_ID, EMP_NAME,JOB_NAME, DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남',2, '여') AS 성별, EXTRACT(YEAR FROM SYSDATE) -EXTRACT(YEAR FROM HIRE_DATE)||'년' AS 근무년수
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
-- 뷰 삭제
...
UPDATE VW_EMP_SAL
SET SALARY = 7000000
WHERE EMP_ID = 200; -- 성공 (산술연산과 무관한 컬럼은 데이터 변경 가능)vw_job.job_name
